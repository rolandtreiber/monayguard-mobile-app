...
const TagDialog = ({id, updated, type}) => {
  const dispatch = useDispatch()
  const visible = useSelector((state) => state.dialog.dialogVisibilityData.manageTag)
  const [error, setError] = useState('')
  const {createTag, updateTag, getTag} = useContext(APIContext)
