...
import SavingGoalViewModal from "../../components/modals/saving-goal-view-modal";

const SavingsGoals = () => {
  const {getSavingGoals, deleteSavingGoal} = useContext(APIContext)
  ...
  const edit = (id) => {
    setId(id)
    dispatch(showModal('manageSavingGoal'))
  }
  ...
  return (
    <>
      ...
      <Fab
        ...
        onPress={create}
      />
      <SavingGoalDialog id={id} updated={updated}/>
      <SavingGoalViewModal id={id} visible={viewModalVisible} setVisible={setViewModalVisible}/>
      <ConfirmationDialog body={"You are about to delete a saving goal"} callback={commitDelete} visible={deleteDialogVisible} setVisible={setDeleteDialogVisible}/>
    </>
  )
}
