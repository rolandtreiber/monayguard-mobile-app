...
import GuardViewModal from "../../components/modals/guard-view-modal";

const Guards = () => {
  const {getGuards, deleteGuard} = useContext(APIContext)
  ...
  const edit = (id) => {
    setId(id)
    dispatch(showModal('manageGuard'))
  }
  ...
  return (
    <>
      ...
      <Fab
        ...
        onPress={create}
      />
      <GuardDialog id={id} updated={updated}/>
      <GuardViewModal id={id} visible={viewModalVisible} setVisible={setViewModalVisible}/>
      <ConfirmationDialog body={"You are about to delete a guard"} callback={commitDelete} visible={deleteDialogVisible} setVisible={setDeleteDialogVisible}/>
    </>
  )
}
