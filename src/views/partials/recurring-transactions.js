...
import {Divider, List, Paragraph, TextInput, Title} from "react-native-paper";
import {ButtonGroup, Fab, FlexContainer, RoundButton, Row, SmallText} from "../../components/shared-styled-components";
import {FlatList} from "react-native";
import ConfirmationDialog from "../../components/dialogs/confirmation-dialog";
import {APIContext} from "../../context/api-context";
import {useDispatch} from "react-redux";
import {showModal} from "../../redux/actions/dialogActions";
import RecurringTransactionDialog from "../../components/dialogs/recurring-transaction-dialog";
import RecurringTransactionViewModal from "../../components/modals/recurring-transaction-view-modal";

const RecurringTransactions = () => {
  const {getRecurringTransactions, deleteRecurringTransaction} = useContext(APIContext)
  ...
  const commitDelete = () => {
    deleteRecurringTransaction(id).then(() => {
      fetchData()
    })
  }
  ...
  return (
    <>
      ...
      <Fab
        ...
        onPress={create}
      />
      <RecurringTransactionDialog id={id} updated={updated}/>
      <RecurringTransactionViewModal id={id} visible={viewModalVisible} setVisible={setViewModalVisible}/>
      <ConfirmationDialog body={"You are about to delete a recurring transaction"} callback={commitDelete} visible={deleteDialogVisible} setVisible={setDeleteDialogVisible}/>
    </>
  )
}
